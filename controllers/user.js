...
const getUser = async function (id) {
  ...
  if (user.login) {
    await promisePool.execute(`SELECT password, reduction, CONVERT(DECODE(password, 'abills345678901234490137') USING utf8) as password FROM users WHERE uid = ${id}`).then(handleResolve, handleReject);

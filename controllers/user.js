...
const getUser = async function (id) {
  ...
  await promisePool.execute(`SELECT id as login, password, reduction, disable as isExpired, CONVERT(DECODE(password, 'abills345678901234490137') USING utf8) as password FROM users WHERE uid = ${id}`).then(handleResolve, handleReject);
